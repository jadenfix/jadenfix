# Jaden Fix

<p align="center">
  <img[image (6)](https://github.com/user-attachments/assets/f8f0efb9-edc5-48e2-a5fd-f6c9fca437b1)
tor"/>
</p>

### AI Agent Engineer · Distributed Systems · Autonomous Infrastructure  
AI Agent Engineer @ Roe (YC F24) | San Francisco Bay Area

---

## About

I design and deploy production AI agents that operate in enterprise environments.

- Multi-step, Temporal-orchestrated agent systems  
- Compliance, AML, and risk automation  
- Audit-first, artifact-driven architecture  
- Deep integration with real infrastructure  

I build and deploy systems that **reason, retrieve, validate, and act**.

---

## Focus

- Agent orchestration & long-running workflows  
- Structured extraction & schema-constrained generation  
- Risk intelligence & adverse media detection  
- Event-driven automation (Slack → Warehouse → Ticketing → Disposition)  
- Production API & distributed system design  

---

## Stack

**LLMs:** Claude, GPT, tool-using agents, RAG  
**Infra:** AWS, EKS, Vercel, Terraform  
**Data:** ClickHouse, AWS
**Backend:** Python, TypeScript, Go, Rust  
**Frontend:** Next.js, React  

---

## Philosophy

- Traceable systems  
- Deterministic guardrails  
- Structured outputs  
- Horizontally scalable design  

---

## Connect

LinkedIn: https://linkedin.com/in/jadenfix  
Location: San Francisco Bay Area  

---

<p align="center"><em>Build agents that move through data like light through fiber.</em></p>
